...
class ChatBot:
    ...
    def send_prompt(self, prompt, temperature=0.1):
        if temperature < 0 or temperature > 1:
            raise GeniAIException('temperature must be between 0 & 1')
        if not prompt:
    ...
    def preload_conversation(self, conversation_history = None):
        if isinstance(conversation_history,list):
            ...
            self._conversation_history = [ 
                ...
                self._construct_message('{"text" : "Sure i acn return the output as regular JSON object with the key as `text`. here is an example:{"text": "your output"}.', 'model')
            ]